...
\label{chap:agn}
Compared to ordinary galaxies, some have extraordinarily bright central regions, which can sometimes even outshine the host-galaxy emission by many orders of magnitude; these galaxies are called \textit{active galaxies} and their nuclei are referred to as \textit{active galactic nuclei}. The physical processes necessary to create such high luminosities ($L = 10^{43-48} \text{erg s}^{-1}$; e.g., \cite{Lusso2012AGNLuminosity}) within such a small volume were a subject of debate for years, but now are widely accepted to be connected to accretion onto the galaxies' central black holes (BHs; \cites{Salpeter1964AccretionTheory}{Zeldovich1964AccretionTheory}). These accreting objects have masses $M_{BH} \sim 10^{6-10} M_{\odot}$ (e.g., \cite{Peterson2004AstNach}) and an Eddington ratio exceeding $\lambda_{Edd}$\footnote{The ratio $L_/L_{Edd}$, where $L$ is the bolometric luminosity and $L_{Edd} = 1.5 \times 10^{38}M_{BH}/M_{\odot} \text{ erg s}^{-1}$ is the Eddington luminosity for a solar composition gas. This is the maximum isotropic luminosity a body can achieve while maintaining balance between radiation pressure and gravitational force.} $\approx 10^{-5}$. This threshold, chosen somewhat arbitrarily, excludes galactic nuclei with no significant accretion activity, such as that of the Milky Way, while still covering their full range of observed luminosities \parencite{Netzer2015AnnRev}. 
...
%The emission from each of the components described above varies in time, and these brightness fluctuations carry information about the physical structure of the accretion flow on scales that cannot be spatially resolved. Their timescales and amplitudes relate directly to the size and attributes of the emitting region, but reconstructing the signal across these gaps is an inherently underdetermined problem, which motivates a probabilistic treatment that returns a distribution over the plausible light curves rather than a single estimate. 

%This thesis investigates the use of \textit{neural stochastic flows} for this task, with the aim of validating the method against the analytic properties of the process used to model the variability and characterizing its behaviour on simulated and real light curves, and it is organized as follows. Chapter \ref{chap:variability} reviews the physical origin of AGN variability and the tools used to characterize it. Chapter \ref{chap:simulation} sets out the simulation framework, damped random walk and the survey regimes reproduced in the simulations. Chapter \ref{chap:nsf} presents the reconstruction method, developing the neural stochastic flows and the bridge construction used to fill the gaps. Chapter \ref{chap:reconstruction} reports the results on simulated and real ZTF light curves, and Chapter \ref{chap:conclusions} summarizes the findings and outlines future work.