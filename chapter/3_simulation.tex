...
This chapter describes the framework used to generate realistic AGN light curves with which the reconstruction method of Chapter \ref{chap:models} is trained and evaluated. The intrinsic variability is modeled using the \textsc{EzTao} Gaussian process library \parencite{Yu2022Extao}, which is then observed through an irregular, gappy cadence with added photometric noise, mimicking the sampling of a ground-based monitoring campaign.
...
where $\tau_\mathrm{DRW}$ is the damping (or characteristic) timescale, $\sigma_\mathrm{DRW}$ is the asymptotic variability amplitude, and $\epsilon(t)$ is a white noise process.

\begin{equation}
    X_{t+\Delta t} \mid X_t \;\sim\; \mathcal{N}\!\left(\mu + e^{-\Delta t/\tau}\left(X_t - \mu\right),\; \sigma^2\left(1 - e^{-2\Delta t/\tau}\right)\right),
    \label{eq:drw_transition}
\end{equation}

The DRW has an exponential autocorrelation function:
...
%The DRW parameters $\tau_\mathrm{DRW}$ and $\sigma_\mathrm{DRW}$ depend on the observing wavelength, black hole mass, and luminosity. Following \textcite{MacLeod2010ScalRel}, the wavelength dependence can be expressed as power-law scaling relations:
...
w%here $\lambda$ is the effective wavelength of the observing filter. These relations imply that redder bands have slightly longer damping timescales and lower variability amplitudes than bluer bands, consistent with the wavelength dependence of AGN variability discussed in Chapter~\ref{chap:variability}.

%In the simulation framework, a \textit{driver band} is defined as the reference band from which the DRW parameters of all other bands are derived via Equation~\ref{eq:MacLeod}. The driver band parameters ($\tau_\mathrm{DRW}^g = 100$ days, $\sigma_\mathrm{DRW}^g = 0.112$ mag) are taken as fiducial values representative of typical quasar optical variability \parencite{Yu2025ScalableEztao}, with the $g$ band chosen as the reference. The parameter space explored in this work follows \textcite{Yu2025ScalableEztao}:
...
\subsection{Damping Timescale}
\label{ssec:TauPrior}

Rather than sampling $\tau$ from an arbitrary range, we draw it from the distribution implied by the population of AGN black-hole masses. \textcite{Burke2021TimescaleBHMass} established a correlation between the optical damping timescale and the black-hole mass,
\begin{equation}
    \tau_{\mathrm{damping}} = 107^{+11}_{-12}\ \mathrm{days}\,\left(\frac{M_{\mathrm{BH}}}{10^{8}\,M_\odot}\right)^{0.38^{+0.05}_{-0.04}},
    \label{eq:burke}
\end{equation}
which we combine with the bias-corrected black-hole mass distribution of \textcite{Sun2023QuasarMassDist} for their preferred model. Each simulated curve draws a value of $\log M_{\mathrm{BH}}$ from the \textcite{Sun2023QuasarMassDistDist} distribution by inverse-transform sampling, which is then mapped to a damping timescale through Equation~\ref{eq:burke}. Sampling the full distribution, including its high-mass tail, yields a training set whose $\tau$ distribution reflects the physical quasar population rather than a uniform prior; 
%the induced distribution has a median of $\tau \approx \SI{XXX}{\day}$ with a central $68\%$ interval of $[\SI{XXX}{}, \SI{XXX}{}]\ \si{\day}$ (Figure~\ref{fig:tau_prior}). 

\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{assets/5_figures/FixedTauExp/TauDist.pdf}
    \caption[Characteristic Timescale]{\label{fig:tau_prior} .}
\end{figure}
